...
function Portraits() {
    ...
    // This useEffect was added so that step 1 is transitioned into the page on pageload regardless of any conditions.
    useEffect(() => {
      // Curation of the transition in milliseconds.
      const transitionDuration = 1000;
      // Number of frames per second for the animation.
      const framesPerSecond = 60;
      // Calculate the increment for changing opacity per frame.
      const opacityIncrement = 1 / (transitionDuration / (1000 / framesPerSecond));
      // Calculate the increment for changing translateY per frame.
      const translateYIncrement = -2 / (transitionDuration / (1000 / framesPerSecond));
      let interval;
      interval = setInterval(() => {
        // Update 'opacity' state by incrementing it if it's less than 1.
        setOpacity((prevOpacity) => {
          if (prevOpacity < 1) {
            return prevOpacity + opacityIncrement;
          } else {
            // When opacity reaches 1, clear the interval to stop the animation.
            clearInterval(interval);
            return 1;
          }
        });
    
        // Update 'translateY' state by incrementing it if it's less than 2.
        setTranslateY((prevTranslateY) => {
          if (prevTranslateY < 2) {
            return prevTranslateY + translateYIncrement;
          } else {
            // When 'translateY' reaches 2, set it to -2 to restart the animation.
            return -2;
          }
        });
      }, 1000 / framesPerSecond);
      // Cleanup function: clear the interval when the component unmounts or if the dependencies change.
      return () => {
        clearInterval(interval);
      };
    ...
    // This useEffect was added so at certain viewport widths and heights, step 2 is transitioned into the page on pageload for a better aesthetic appearance.  It is nearly identical to the useEffect above.
    useEffect(() => {
